...
export const generateScriptForStage = async (
  ...
  if (stage === TeachingStage.Import) {
    ...
    stageConstraint = "本环节时长约10-15分钟。建议生成4-6个分镜（编号）。飞飞博士带领学生解决问题。";
    ...
    stageConstraint = "本环节时长约3分钟。建议生成2-3个分镜（编号）。支持生成“交互题”。";
  } else if (stage === TeachingStage.Share) {
    stageConstraint = `
    【分享展示环节特殊要求】：
    1. 内容包含：邀请分享、回顾知识、手册答题（3道题）、学生自评。
    2. **强制新增任务**：在完成手册题目/自评的流程中，必须有一句台词引导学生“填写AI芯片卡背后的AI词汇”。
    `;
...
export const modifyScript = async (
  ...
  const prompt = `
    ...
    2. **内容约束**：
       ...
       - 如果是“分享展示”环节，**必须保留**关于“AI芯片卡词汇”的引导。
    3. **格式约束**：
